...
<xsl:stylesheet
    ...
    <xsl:template match="/">
        <xsl:if test="not(function-available('exsl:node-set'))">
            <xsl:message terminate="yes">FATAL ERROR: exsl:node-set function is not available in this XSLT processor
            </xsl:message>
        </xsl:if>
        <!--<xsl:if test="not(function-available('str:replace'))">-->
        <!--<xsl:message terminate="yes">FATAL ERROR: str:replace function is not available in this XSLT processor-->
        <!--</xsl:message>-->
        <!--</xsl:if>-->
        <!--<xsl:if test="not(function-available('dyn:evaluate'))">-->
        <!--<xsl:message terminate="yes">FATAL ERROR: dyn:evaluate function is not available in this XSLT processor-->
        <!--</xsl:message>-->
        <!--</xsl:if>-->
        ...
        <xsl:for-each select="/h:html/h:head/xf:model/xf:bind">
            <xsl:if test="not(substring(./@nodeset, 1, 1) = '/')">
                <xsl:message terminate="no">WARNING: Found binding(s) with relative (= bad!) nodeset
                    attribute <!--on element: <xsl:value-of select="./@nodeset" />--> (form may work correctly if
                    relative nodesets were used consistently throughout xml form in bindings as well as body, otherwise
                    it will certainly be messed up).
                </xsl:message>
            </xsl:if>
        </xsl:for-each>
        ...
        <root>
            ...
            <form class="jr" autocomplete="off">
                <xsl:attribute name="id">
                    <xsl:choose>
                        <xsl:when test="/h:html/h:head/xf:model/xf:instance[1]/child::node()/@id">
                            <xsl:value-of
                                    select="translate(/h:html/h:head/xf:model/xf:instance/child::node()/@id, ' ', '_' )"/>
                            <!-- not smart! -->
                        </xsl:when>
                        <xsl:when test="/h:html/h:head/xf:model/xf:instance/child::node()/@xmlns">
                            <xsl:value-of
                                    select="translate(/h:html/h:head/xf:model/xf:instance/child::node()/@xmlns, ' ', '_')"/>
                        </xsl:when>
                        <xsl:otherwise>
                            <xsl:text>_</xsl:text>
                        </xsl:otherwise>
                ...
                <xsl:text>&#10;</xsl:text>
                <xsl:comment>This form was created by transforming a javaRosa-style (X)Form using an XSLT sheet created
                ...
                <xsl:if test="/h:html/h:body//xf:output">
                    <xsl:message>WARNING: Output element(s) added but note that only /absolute/path/to/node is properly
                        supported as "value" attribute of outputs. Please test to make sure they do what you want.
                    </xsl:message>
                </xsl:if>
                <xsl:if test="/h:html/h:body//xf:itemset">
                    <xsl:message>WARNING: Itemset support is experimental. Make sure to test whether they do what you
                        want.
                    </xsl:message>
                </xsl:if>
    ...
    <xsl:template name="copy-model">
        <instance>
            <xsl:copy-of select="//xf:model/xf:instance/*"/>
        </instance>
    </xsl:template>
    ...
    <xsl:template match="xf:repeat">
        ...
        <xsl:comment>end of repeat fieldset with name
            <xsl:value-of select="@nodeset"/>
        </xsl:comment>
    </xsl:template>
    ...
    <xsl:template match="xf:input | xf:upload | xf:item | xf:bind[@calculate]">
        ...
        <label>
            ...
            <xsl:variable name="element">
                <xsl:choose>
                    <xsl:when
                            test="$binding/@type = 'string' and $appearance = 'multi-line' or $appearance = 'multiline' or $appearance = 'text-area' or $appearance = 'big' or $appearance = 'big-text' or $appearance = 'textarea'">
        ...
        <span class="itemset-labels">
            ...
            <xsl:choose>
                <xsl:when test="contains($label-ref, 'jr:itext(')">
                    ...
                    <xsl:for-each
                            select="dyn:evaluate(concat('/h:html/h:head/xf:model/xf:instance[@id=&quot;', $instance-id, '&quot;]', $instance-path-nofilter))">
    ...
    <xsl:template match="xf:trigger">
        ...
        <xsl:message>WARNING: Trigger(s) added but but with little support. Please test to make sure they do what you
            want.
        </xsl:message>
    </xsl:template>
        ...
        <xsl:variable name="nodeset_a">
            <xsl:choose>
                <xsl:when test="not(substring($nodeset_u, 1, 1) = '/')">
                    <xsl:value-of
                            select="concat('/', local-name(/h:html/h:head/xf:model/xf:instance/child::*[1]), '/', $nodeset_u)"/>
    ...
    <xsl:template name="html_type">
        ...
        <xsl:choose>
            ...
            <xsl:when test="$xml_type = 'boolean'">checkbox</xsl:when>
            ...
            <!-- temporary -->
            <xsl:when test="$xml_type = 'geopoint'">
            ...
            <xsl:otherwise>
                ...
                <xsl:message terminate="no">ERROR: Unsupported data type '<xsl:value-of select="$xml_type"/>' found.
                </xsl:message>
            </xsl:otherwise>
        </xsl:choose>
    </xsl:template>
