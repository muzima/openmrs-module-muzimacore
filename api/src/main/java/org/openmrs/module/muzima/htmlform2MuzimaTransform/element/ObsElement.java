			...
			if (concept.getDatatype().isNumeric()) {
				...
				if (numericAnswers.size() == 0) {
					...
					for (int i = 0; i < numericAnswers.size(); i++) {
						...
						((SingleOptionField) valueField).addOption(
				...
				if (answerConcept != null) {
					...
					if (isAutocomplete) {
						...
						if (StringUtils.isNotBlank(answerDrugId)) {
							...
							valueField = createToggleCheckbox(concept, null, "Drug:" + drug.getId().toString(), locale,
								valueLabel, drug.getName(), null);
						}
					} else {
				...
				if ("checkbox".equals(parameters.get("style"))) {
					valueField = createToggleCheckbox(concept, parameters.get("value") != null ? noConcept : yesConcept,
						null, locale, valueLabel, parameters.get("value") != null ? noStr : yesStr,
						parameters.get("toggle"));
					valueLabel = "";
...
//		 if a date is requested, do that too
	...
	private FormField createToggleCheckbox(Concept concept, Concept ansConcept, String ansValue, Locale locale,
		...
		if (toggleParameter != null) {
			...
			if (ansConcept != null) {
				...
				cbf.addOption(new Option(ansConcept, locale, false));
				return cbf;
			} else {
				CheckboxField cbf = new CheckboxField(concept, locale, fieldLabel);
