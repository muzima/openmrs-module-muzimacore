...
import org.openmrs.module.xforms.formentry.FormEntryException;
 ...
 */
public class HtmlFormConversionUtil {
	
	public static Log log = LogFactory.getLog(HtmlFormConversionUtil.class);
	...
	/**
	 * Fetches a location tag by name or id (Will add support for uuid once we stop supporting
	 * OpenMRS 1.6, which doesn't a uuid on location tag)
	 *
	 * @param identifier
	 * @return
	 */
	...
	/**
	 * Given a list of patient programs and a program, returns the first patient program that
	 * matches the specified program
	 *
	...
	/**
	 * Given a patient and a program workflow, returns the first patient program that contains a
	 * state in the specified workflow
	 *
	 * @param patient
	 * @param workflow
	 * @return
	...
	/**
	 * If the specified patient is enrolled in the specified program on the specified date, return
	 * the associated patient program, otherwise return null
	 *
	 * @param patient
	 * @param program
	 * @param date
	...
	/**
	 * Formats a piece of Metadata for Display TODO This was copied from UiFramework--we probably
	 * should come up with a way to inject a formatter directly into HFE
	 *
	 * @param md
	 * @param locale
	 * @return
	...
	/**
	 * Attempts to parse the passed string as an Integer and fetch the provider role with that id If
	 * no match, or the string is unparseable as an Integer, try to fetch by uuid
	 *
	 * @param id
	 * @return
	 */
	...
	/**
	 * @return the provider with the given id, where the id can be either the primary key id or uuid
	 *         of the provider If id passed in is "currentuser", then return the first provider
	 *         record associated with the currently authenticated user
	 */
	 ...
	 * with family name compoenents first, separated by a comma from given and middle names. If any
	 * part of {@link PersonName#getPrefix()}, {@link PersonName#getGivenName()},
