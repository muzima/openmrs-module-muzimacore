...
public class MuzimaFormServiceImplTest extends BaseModuleContextSensitiveTest {
	...
	public void setUp() throws Exception {
		...
		syncDate = new Date();
		//TODO update tests
		service = new MuzimaFormServiceImpl(dao, transformer, modelTransformer, odk2JavarosaTransformer,
		        odk2HTML5Transformer, null);
	}
	
	void setUpDao() {
		List<MuzimaForm> muzimaForms = new ArrayList<MuzimaForm>();
		muzimaForms.add(muzimaform().withId(1)//.withName("Registration Form").withDescription("Form for registration")
		        .with(tag().withId(1).withName("Registration")).with(tag().withId(2).withName("Patient")).instance());
		muzimaForms.add(muzimaform().withId(2)//.withName("PMTCT Form").withDescription("Form for PMTCT")
		        .with(tag().withId(1).withName("Registration")).with(tag().withId(3).withName("Encounter"))
		        .with(tag().withId(4).withName("HIV")).instance());
	...
	public void shouldNotCreateHTMLFormIfFormNameAlreadyExists() throws Exception {
		List<MuzimaForm> muzimaForms = asList(getMuzimaFormWithName("Something like name"), getMuzimaFormWithName("name"));
	...
	public void shouldUpdateTheHTMLFormOfAnExistingForm() throws Exception {
		...
		MuzimaForm registrationForm = muzimaform().withId(1)//.withName("Registration Form").withDescription("Form for registration")
		        .with(tag().withId(1).withName("Registration")).with(tag().withId(2).withName("Patient"))
		...
		MuzimaForm pmtctForm = muzimaform().withId(2)//.withName("PMTCT Form").withDescription("Form for PMTCT")
		        .with(tag().withId(1).withName("Registration")).with(tag().withId(3).withName("Encounter"))
		        .with(tag().withId(4).withName("HIV")).withForm("c0c579b0-8e59-401d-8a4a-976a0b183522")
		        .withFormDefinition(Context.getFormService().getFormByUuid("c0c579b0-8e59-401d-8a4a-976a0b183522"))
		        .instance();
	...
	public void importExisting_shouldRetrieveExistingXFormAndConvertItIntoHTML5AndPersistAMuzimaForm() throws Exception {
		...
		MuzimaForm testForm = service.create(xformsService.getXform(1).getXformXml(), "c0c579b0-8e59-401d-8a4a-976a0b183519",
		    "discriminator");
		
		verify(dao, times(1)).saveForm(muzimaform().withUuid(testForm.getUuid()).withDiscriminator("discriminator")
	...
	public void importExisting_shouldSetConvertedXform() throws Exception {
		...
		verify(dao, times(1)).saveForm(
		    muzimaform().withForm("form").withDiscriminator("discriminator").withForm("c0c579b0-8e59-401d-8a4a-976a0b183522")
		            .withFormDefinition(Context.getFormService().getFormByUuid("c0c579b0-8e59-401d-8a4a-976a0b183522"))
		            .instance());
	...
	@Ignore
	@Test(expected = DocumentException.class)
	public void shouldNotCreateFormIfTheNameAlreadyExists()
	        throws ParserConfigurationException, TransformerException, DocumentException, IOException {
		List<MuzimaForm> muzimaForms = asList(getMuzimaFormWithName("Something like name"), getMuzimaFormWithName("name"));
		when(dao.getFormByName("name", syncDate)).thenReturn(muzimaForms);
		try {
	...
	@Test
	public void shouldCreateFormIfSimilarNameExistsButNotExactMatch()
	        throws ParserConfigurationException, TransformerException, DocumentException, IOException {
